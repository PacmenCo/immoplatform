...
import { MarkPaidButton } from "@/app/[locale]/dashboard/commissions/MarkPaidButton";
...
export default async function OverviewPage({
  ...
  if (!hasRole(session, "admin")) {
    await localeRedirect("/no-access?section=revenue");
  }

  const t = await getTranslations("dashboard.overview");
  const tStats = await getTranslations("dashboard.overview.stats");
  const tPeriodTabs = await getTranslations("dashboard.overview.periodTabs");
  const tMonths = await getTranslations("dashboard.overview.months");
  const tByMonth = await getTranslations("dashboard.overview.byMonth");
  ...
  return (
    <>
      <Topbar title={t("topbarTitle")} subtitle={periodLabel(period)} />
      ...
      <div className="p-8 max-w-[1400px] space-y-6">
        <div className="flex flex-wrap items-center gap-4">
          <PeriodTabs
            ...
            now={now}
            labels={{
              month: tPeriodTabs("month"),
              quarter: tPeriodTabs("quarter"),
              year: tPeriodTabs("year"),
            }}
          />
          <YearPicker period={period} years={yearOptions} />
          {period.kind === "quarter" && <QuarterPicker period={period} />}
          {period.kind === "month" && <MonthPicker period={period} monthShort={monthShort} />}
          ...
          <StatCard
            ...
            hint={
              totals.adjustmentCents !== 0
                ? tStats("revenueHintCompletedWithAdj", {
                    count: totals.assignmentCount,
                    adjustment: formatEuros(totals.adjustmentCents),
                  })
                : tStats("revenueHintCompleted", { count: totals.assignmentCount })
            }
          />
          <StatCard
            label={tStats("commissionAccrued")}
            value={formatEuros(totals.commissionAccruedCents)}
            hint={tStats("commissionAccruedHint")}
          />
          <StatCard
            label={tStats("commissionPaid")}
            value={formatEuros(totals.commissionPaidCents)}
            hint={tStats("commissionPaidHint")}
            tone="ok"
          />
          <StatCard
            label={tStats("outstanding")}
            value={formatEuros(outstanding)}
            hint={tStats("outstandingHint")}
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <RevenueBarChart
            ...
            title={tByMonth("title")}
            monthsCountLabel={tByMonth("monthsCount", { count: byMonth.length })}
          />
          <ByServiceCard
            byService={byService}
        ...
        <Card>
          <CardHeader className="flex items-center justify-between gap-3">
            <div>
              <CardTitle>{tByTeam("title")}</CardTitle>
              <p className="mt-1 text-sm text-[var(--color-ink-muted)]">
                {tByTeam("subtitle", { period: periodLabel(period) })}
              </p>
            </div>
            <Link
          ...
          <CardBody className="p-0">
            {sortedByTeam.length === 0 ? (
              <EmptyState
                ...
                title={tByTeam("emptyTitle", { period: periodLabel(period) })}
                description={
                  teamId
                    ? tByTeam("emptyDescriptionFiltered")
                    : tByTeam("emptyDescriptionDefault")
                }
              />
            ) : (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-[var(--color-border)] bg-[var(--color-bg-alt)] text-xs uppercase tracking-wider text-[var(--color-ink-muted)]">
                      <ByTeamSortHeader id="team" label={tByTeam("headerTeam")} currentSort={sort} currentDir={dir} />
                      <ByTeamSortHeader id="assignments" label={tByTeam("headerAssignments")} currentSort={sort} currentDir={dir} align="right" />
                      <ByTeamSortHeader id="revenue" label={tByTeam("headerRevenue")} currentSort={sort} currentDir={dir} align="right" />
                      <ByTeamSortHeader id="commission" label={tByTeam("headerCommission")} currentSort={sort} currentDir={dir} align="right" />
                      <th className="px-6 py-3 text-right font-semibold">{tByTeam("headerPaid")}</th>
                      <th className="px-6 py-3 text-left font-semibold">{tByTeam("headerStatus")}</th>
                      {period.kind === "quarter" && (
                        <th className="px-6 py-3 text-right font-semibold">{tByTeam("headerPayout")}</th>
                      )}
                    </tr>
                  </thead>
                        ...
                        <tr key={row.teamId} className="hover:bg-[var(--color-bg-alt)]">
                          ...
                          <td className="px-6 py-3">
                            {row.commissionAccruedCents === 0 && row.commissionPaidCents === 0 ? (
                              <span className="text-xs text-[var(--color-ink-muted)]">{tByTeam("statusNoCommission")}</span>
                            ) : fullyPaid ? (
                              ...
                              <span className="text-xs text-[var(--color-electrical)]">{tByTeam("statusPartial")}</span>
                            ) : (
                              <span className="text-xs text-[var(--color-ink-muted)]">{tByTeam("statusOutstanding")}</span>
                            )}
                          </td>
                          {period.kind === "quarter" && (
                            <td className="px-6 py-3 text-right">
                              ...
                              {row.commissionAccruedCents > 0 ? (
                                ...
                                <span className="text-xs text-[var(--color-ink-muted)]">{tByTeam("noPayoutDash")}</span>
                              )}
                            </td>
                          )}
                        </tr>
...
function MonthPicker({ period, monthShort }: { period: Extract<Period, { kind: "month" }>; monthShort: string[] }) {
  return (
  ...
  return (
    <Card className={"p-6 " + (className ?? "")}>
      <div className="flex items-baseline justify-between">
        ...
        <p className="text-xs text-[var(--color-ink-muted)]">{monthsCountLabel}</p>
      </div>
    ...
    <Card>
      ...
      <CardBody className="space-y-4">
        ...
        <div className="flex items-baseline justify-between border-t border-[var(--color-border)] pt-4">
          <span className="text-sm text-[var(--color-ink-muted)]">{totalLabel}</span>
