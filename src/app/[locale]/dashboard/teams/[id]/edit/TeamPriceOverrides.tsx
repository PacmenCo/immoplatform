...
export function TeamPriceOverrides({
  ...
  const t = useTranslations("dashboard.teams.edit.priceOverrides");
  return (
    <>
      {odooError && (
        <p
      ...
      <div className="grid grid-cols-[auto_1fr_auto_auto] items-center gap-4">
        ...
        <div className="min-w-0">
          ...
          <p className="text-xs text-[var(--color-ink-muted)]">
            {t("basePrice", { price: formatEuros(row.basePriceCents) })}
          </p>
          {error && (
            <div className="mt-2">
        ...
        <div className="flex items-center gap-1">
          ...
          <input
            ...
            aria-label={t("overrideAriaLabel", { label: row.label })}
          />
        </div>
            ...
            <div className="max-h-72 overflow-y-auto rounded-md border border-[var(--color-border)]">
              <table className="w-full text-sm">
                <thead className="sticky top-0 bg-[var(--color-bg-alt)] text-[10px] uppercase tracking-wider text-[var(--color-ink-muted)]">
                  <tr>
                    <th className="px-3 py-2 text-left font-medium">{t("itemsColumnProduct")}</th>
                    <th className="px-3 py-2 text-right font-medium">{t("itemsColumnPrice")}</th>
                  </tr>
                </thead>
                <tbody>
                  {items.map((it) => (
