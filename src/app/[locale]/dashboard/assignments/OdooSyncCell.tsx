...
export function OdooSyncCell({
  ...
  const t = useTranslations("dashboard.assignments.odooSync");
  ...
  if (isWarning) {
    return (
      <span
        ...
        aria-label={odooSyncError ?? t("warningFallback")}
      >
  ...
  const tooltip = isFailed
    ? (odooSyncError ?? t("failedFallback"))
    : t("pendingTooltip");
