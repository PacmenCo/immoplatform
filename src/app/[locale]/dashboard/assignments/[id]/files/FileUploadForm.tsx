...
export function FileUploadForm({
  ...
  const t = useTranslations("dashboard.assignments.files");
  ...
  async function runUpload(batch: File[]) {
    ...
    try {
      ...
      const xhrMessages = {
        ...
        uploadAborted: t("uploadAborted"),
        uploadFailedStatus: (status: number) => t("uploadFailedStatus", { status }),
      };
      const workers = Array.from({ length: Math.min(UPLOAD_CONCURRENCY, queue.length) }, async () => {
        while (queue.length > 0) {
          ...
          try {
            ...
            }, inFlight.current, xhrMessages);
            completed.push(next.u);
          } catch (err) {
            failed.push(next.u);
            if (!firstError) {
              firstError = err instanceof Error ? err.message : t("uploadFailed");
            }
          }
        }
      });
      ...
      if (failed.length > 0) {
        ...
        const msg = firstError ?? t("filesFailedFallback");
        setError(msg);
        toast.error(msg);
        return;
      }
      ...
      if (!fin.ok) {
        setError(fin.error);
        toast.error(tErr(fin.error));
        return;
      }

      const n = batch.length;
      toast.success(t("uploadedToast", { count: n }));
      setFiles([]);
      setProgress([]);
  ...
  return new Promise((resolve, reject) => {
    ...
    xhr.onload = () => {
      ...
      if (xhr.status >= 200 && xhr.status < 300) {
        ...
        reject(new Error(messages.uploadFailedStatus(xhr.status)));
      }
    };
    ...
    xhr.onerror = () => fail(new Error(messages.networkError));
    ...
    xhr.upload.onerror = () => fail(new Error(messages.networkError));
    xhr.onabort = () => fail(new Error(messages.uploadAborted));
    xhr.send(file);
  });
}
