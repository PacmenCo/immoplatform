...
export function Notice({ notice }: { notice: string | null }) {
  const t = useTranslations("dashboard.assignments.notices");
  ...
  useEffect(() => {
    ...
    if (!m) return;
    fired.current = true;
    toast[m.kind](t(m.translationKey as Parameters<typeof t>[0]));
    const next = new URLSearchParams(searchParams);
    next.delete(NOTICE_PARAM);
    const qs = next.toString();
    router.replace(qs ? `${pathname}?${qs}` : pathname);
  }, [notice, toast, router, pathname, searchParams, t]);
  return null;
}
