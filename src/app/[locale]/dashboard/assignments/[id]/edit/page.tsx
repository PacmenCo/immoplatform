...
import { KeyPickupEditor } from "../KeyPickupEditor";
...
export default async function AssignmentPage({
  ...
  return (
    <>
      ...
      {/* `pb-28` keeps the last card clear of the fixed Save bar. The bar
          is `position: fixed bottom-0` (lives in AssignmentForm) so it
          doesn't take layout space; this padding is what stops the right
          aside or last comment from sliding under it. */}
      <div className="px-8 pt-8 pb-28 max-w-[1400px]">
        ...
        <div className="grid gap-6 lg:grid-cols-3">
            ...
            {(() => {
              const sideCards = (
                <>
                  <Card>
                    ...
                    <CardBody className="space-y-5">
                      <FileList
                        ...
                        emptyMessage={t("deliverablesEmpty")}
                      />
                  ...
                  <Card>
                    <CardHeader>
                      <CardTitle>{t("realtorUploadsTitle")}</CardTitle>
                      <p className="mt-1 text-sm text-[var(--color-ink-soft)]">
                        {t("realtorUploadsDescription")}
                      </p>
                    </CardHeader>
                    <CardBody className="space-y-5">
                      <FileList
                        files={realtorFileRows}
                        emptyMessage={t("realtorUploadsEmpty")}
                      />
                  ...
                  <Card>
                    <CardHeader className="flex items-center justify-between">
                      ...
                      <span className="text-xs text-[var(--color-ink-muted)]">
                        {t("commentsCount", { count: assignment.comments.length })}
                      </span>
                    </CardHeader>
                    <CardBody className="space-y-5">
          ...
          <aside className="space-y-6 lg:self-start min-w-0 lg:mt-8">
              ...
              <Card>
                ...
                <CardBody className="text-sm">
                  ...
                  <p className="mt-3 text-xs text-[var(--color-ink-muted)]">
                    {t("teamCreatedAt", { date: assignment.createdAt.toISOString().slice(0, 10) })}
                  </p>
                </CardBody>
              </Card>
            )}
            ...
            <Card>
              ...
              <CardBody className="space-y-4 text-sm">
                <KeyPickupEditor
                  ...
                  initial={{
                    requiresKeyPickup: assignment.requiresKeyPickup,
                    locationType: assignment.keyPickupLocationType,
                    address: assignment.keyPickupAddress,
                  }}
                  canEdit={canEdit && !isFreelancer && !terminal}
                />
              ...
              <Card>
                ...
                <CardBody className="space-y-2 text-sm">
                  <div className="flex items-baseline justify-between gap-3">
                    <span className="text-[var(--color-ink-soft)]">{t("commissionRate")}</span>
            ...
            <Card>
              <CardHeader>
                <CardTitle>{t("assignmentFormTitle")}</CardTitle>
                <p className="mt-0.5 text-xs text-[var(--color-ink-muted)]">
                  {t("assignmentFormDescription")}
                </p>
              </CardHeader>
              <CardBody className="space-y-3 text-sm">
                <div className="flex items-center gap-3 rounded-md border border-[var(--color-border)] bg-[var(--color-bg-alt)] p-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-[var(--color-ink-muted)]">
                      {t("assignmentFormFilenameSuffix")}
                    </p>
                  </div>
                </div>
