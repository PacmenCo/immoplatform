...
export function AssignmentFilesButton({
  ...
  const t = useTranslations("dashboard.assignments.filesButton");
  ...
  return (
    <>
      <button
        ...
        aria-label={
          isEmpty
            ? t("ariaLabelEmpty", { reference })
            : t("ariaLabelHasFiles", { reference })
        }
        title={isEmpty ? t("titleEmpty") : undefined}
        className={
...
function FilesModal({
  ...
  return createPortal(
    <div
      role="dialog"
      aria-modal="true"
      aria-label={t("modalAriaLabel", { reference })}
      ...
      >
        <div className="flex items-start justify-between gap-4 border-b border-[var(--color-border)] px-5 py-4">
          <div className="min-w-0">
            <p className="text-sm font-semibold text-[var(--color-ink)]">{t("modalTitle")}</p>
        ...
        <div className="max-h-[60vh] overflow-y-auto p-5">
          {error ? (
            <ErrorAlert>{tErr(error)}</ErrorAlert>
          ) : files === null ? (
    ...
    <div className="flex items-center gap-3 rounded-md border border-[var(--color-border)] bg-[var(--color-bg-alt)] p-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="text-xs text-[var(--color-ink-muted)]">
          {file.lane === "freelancer" ? t("freelancerLane") : t("realtorLane")} ·{" "}
          {formatBytes(file.sizeBytes)}
        </p>
        {error && <p className="mt-1 text-xs text-[var(--color-danger)]">{tErr(error)}</p>}
      </div>
