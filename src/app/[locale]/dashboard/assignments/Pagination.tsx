 ...
 */
export async function Pagination({ current, total, pageSize, buildUrl }: Props) {
  const t = await getTranslations("dashboard.assignments.pagination");
  ...
  return (
    ...
    >
      <p className="text-xs text-[var(--color-ink-muted)]">
        {t("summary", { current, total: totalPages, count: total })}
      </p>
          ...
          p === "…" ? (
            ...
            <li key={p}>
              <PageLink
                href={buildUrl(p)}
                active={p === current}
                aria-label={t("pageNumberAria", { n: p })}
