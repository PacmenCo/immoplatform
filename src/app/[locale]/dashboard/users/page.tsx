...
function relativeFormatterFromMessages(t: (key: string, vars?: Record<string, unknown>) => string) {
  return (from: Date | null, now: Date): string => {
    if (!from) return t("never");
...
export default async function UsersPage({
  ...
  const t = await getTranslations("dashboard.users.list");
  const tRoles = await getTranslations("dashboard.users.roles");
  const tLastSeen = await getTranslations("dashboard.users.list.lastSeen");
  const tLastSeenFn = (key: string, vars?: Record<string, unknown>) =>
    tLastSeen(key as never, vars as never);
  const relativeTime = relativeFormatterFromMessages(tLastSeenFn);
  ...
  return (
    <>
      <Topbar
        title={t("topbarTitle")}
        subtitle={t("topbarSubtitle", { count: users.length })}
      />
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[var(--color-border)] bg-[var(--color-bg-alt)] text-xs uppercase tracking-wider text-[var(--color-ink-muted)]">
                  <th scope="col" className="text-left font-semibold px-6 py-3">{t("columns.user")}</th>
                  <th scope="col" className="text-left font-semibold px-6 py-3">{t("columns.role")}</th>
                  <th scope="col" className="text-left font-semibold px-6 py-3">{t("columns.team")}</th>
                  <th scope="col" className="text-left font-semibold px-6 py-3">{t("columns.joined")}</th>
                  <th scope="col" className="text-left font-semibold px-6 py-3">{t("columns.lastSeen")}</th>
                  <th scope="col" className="text-right font-semibold px-6 py-3"><span className="sr-only">{t("columns.actions")}</span></th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[var(--color-border)]">
                {users.map((u) => {
                  const rb = roleBadge(u.role);
                  const teamName = u.memberships[0]?.team.name ?? t("noTeam");
                    ...
                    >
                      ...
                      <td className="px-6 py-3">
                        <Badge bg={rb.bg} fg={rb.fg}>
                          <span>{tRoles(u.role as "admin" | "staff" | "realtor" | "freelancer")}</span>
                        </Badge>
                      </td>
                      ...
                      <td className="px-6 py-3 text-right">
                        <div className="inline-flex items-center gap-2">
                          ...
                          <Link
                            href={`/dashboard/users/${u.id}`}
                            aria-label={t("viewUserAriaLabel", { name: fullName })}
