...
export function SetPasswordForm({
  ...
  const t = useTranslations("auth.invite.setPassword");
  ...
  const roleLabel = tInvite(`roles.${role as "admin" | "staff" | "realtor" | "freelancer"}`);
      ...
      <div className="space-y-6">
        <div className="rounded-md border border-[var(--color-border)] bg-[var(--color-bg-alt)] p-4">
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-[var(--color-ink-soft)]">
            <Badge bg={rc.bg} fg={rc.fg} size="sm">{roleLabel}</Badge>
            {team && (
              <>
        ...
        <form ref={formRef} className="space-y-5" action={formAction}>
          ...
          <label className="flex items-start gap-2 text-sm text-[var(--color-ink-soft)]">
            ...
            <span>
              {t("termsPrefix")}{" "}
              <a href="/legal/terms" className="underline">{t("termsLink")}</a>{" "}
              {t("termsAnd")}{" "}
              <a href="/legal/privacy" className="underline">{t("privacyLink")}</a>.
            </span>
          </label>
