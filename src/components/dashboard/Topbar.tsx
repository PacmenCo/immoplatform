...
export function Topbar({ title, subtitle }: { title: string; subtitle?: string }) {
  return (
    <header className="hidden lg:flex items-center justify-between gap-4 xl:gap-6 border-b border-[var(--color-border)] bg-white px-6 xl:px-8 h-16 sticky top-0 z-40">
      ...
      <div className="flex flex-1 max-w-md items-center">
        <div className="relative w-full">
          ...
          <input
            ...
            className="w-full h-9 rounded-md border border-[var(--color-border)] bg-[var(--color-bg-alt)] pl-9 pr-12 text-sm placeholder:text-[var(--color-ink-faint)] transition-colors hover:bg-white hover:border-[var(--color-border-strong)] focus:bg-white focus:border-[var(--color-ink-soft)] focus:ring-2 focus:ring-[var(--color-brand)]/10 focus:outline-none"
          />
          <kbd className="pointer-events-none absolute right-2 top-1/2 -translate-y-1/2 hidden xl:inline-flex h-5 items-center gap-0.5 rounded border border-[var(--color-border)] bg-white px-1.5 text-[10px] font-medium text-[var(--color-ink-muted)]">
            ⌘K
          </kbd>
        </div>
      </div>
